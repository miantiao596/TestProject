    ...
    HLSLINCLUDE
        ...
        float Frag(PostProcessVaryings i) : SV_Target
        {
           ...
           //return float4(depth,0,0,0);
            ...
            float isSkyBox;
            #if UNITY_REVERSED_Z
                isSkyBox = (depth < 0.0001);
            #else
                isSkyBox = (depth > 0.99999);
            #endif
            //return half4(isSkyBox,0,0,1);
            totalAtt = lerp(totalAtt,0,_VolumetricLight.w * isSkyBox);
			...
			totalAtt += blueNoise;
            //mali上用的RFloat类型贴图，有可能输出值为负数，导致反而把物体照暗，与其他平台效果不一致
            totalAtt = max(totalAtt, 0);
            return totalAtt;
