...
#ifndef UT_TERRAIN_LIT_PASSES_INCLUDED
#define UT_TERRAIN_LIT_PASSES_INCLUDED
...
Varyings SplatmapVert(Attributes v)
{
    ...
    o.clipPos = Attributes.positionCS;
        ...
        o.shadowCoord = GetShadowCoord(Attributes);
    		...
    		VertexPositionInputs vertexInput = (VertexPositionInputs)0;//雾效接入
				vertexInput.positionWS = o.positionWS;
...
half4 UniversalFragmentPBR_TerrainLitTemp(InputData inputData, SurfaceData surfaceData, float vegAtten)
{
    ...
    return min(half4(finalColor, surfaceData.alpha),HALF_MAX);
    #else
...
half4 UniversalFragmentPBR_TerrainLitTemp(InputData inputData, half3 albedo, half metallic, half3 specular,
    half smoothness, half occlusion, half3 emission, half alpha, half vegAtten)
{
    ...
    return UniversalFragmentPBR_TerrainLitTemp(inputData, surfaceData, vegAtten);
}
...
#endif
{
...
#else
    //山海demo阶段临时代码
    float2 offsetuv = IN.uvMainAndLM.xy + _MainLightPosition.xyz * 0.01 * _SeaGrassShadowOffset;
    offsetuv = (offsetuv - _SeaGrassBound.xy) / (1 - _SeaGrassBound.xy);
    offsetuv = offsetuv / _SeaGrassBound.zw;
    offsetuv = lerp(0, offsetuv, step(0, offsetuv.x));
    offsetuv = lerp(0, offsetuv, step(0, offsetuv.y));
    offsetuv = lerp(0, offsetuv, step(offsetuv.x, 1));
    offsetuv = lerp(0, offsetuv, step(offsetuv.y, 1));
    offsetuv = clamp(offsetuv, 0, 1);
    half seagrassmask = SAMPLE_TEXTURE2D(_SeaGrassDistrobutionMask, sampler_SeaGrassDistrobutionMask, offsetuv).x;
    half disCmr = length(inputData.positionWS - GetCameraPositionWS());
    ...
    half4 color = UniversalFragmentPBR_TerrainLitTemp(inputData, albedo, metallic, /* specular */ half3(0.0h, 0.0h, 0.0h), smoothness, occlusion, /* emission */ half3(0, 0, 0), alpha, vegAtten);

    SplatmapFinalColor(color, inputData.fogCoord);
   

#ifdef _PIXELFOG_ON
    CustomMixFogColor(inputData.positionWS, IN.fogColor.xyz, IN.fogColor.w);//雾效接入
#endif

#ifdef TERRAIN_SPLAT_ADDPASS
    color.rgb = lerp(half3(0,0,0),color.rgb,IN.fogColor.w);
#else
    color.rgb = lerp(IN.fogColor.xyz,color.rgb,IN.fogColor.w);
#endif


    outColor = half4(color.rgb, 1.0h);

#ifdef _TERRAINLAYERNUM_DEBUG
    int layerNum = 0;
    half weightVal;
    for (int i = 1; i < 4; i ++)
    {
        for(int j = 0; j < 4; j++)
        {
            if (i == 1)
            {
                weightVal = SAMPLE_TEXTURE2D(_WeightMap1, sampler_WeightMap1, splatUV)[j];
            }
            else if(i == 2)
            {
                weightVal = SAMPLE_TEXTURE2D(_WeightMap2, sampler_WeightMap1, splatUV)[j];
            }
            else if(i == 3)
            {
                weightVal = SAMPLE_TEXTURE2D(_WeightMap3, sampler_WeightMap1, splatUV)[j];
            }
            else
            {
                weightVal = SAMPLE_TEXTURE2D(_WeightMap4, sampler_WeightMap1, splatUV)[j];
            }
            layerNum = weightVal > 0 ? layerNum + 1 : layerNum + 0;
        }
    }
    outColor = layerNum < 4 ? outColor : half4(1,0,0,1.0h);
#endif    
