#ifndef ATOMSPHERETRANSPARENTLIB_INCLUDE
#define ATOMSPHERETRANSPARENTLIB_INCLUDE
...
half4 frag (Varyings input) : COLOR
{
    ...
    //half4 color = half4(_Color.rgb * _Intensity * albedo.rgb * albedo.a, 1) * mask1 * mask2 * 2.5;
                
    half4 color = input.color * _Intensity * albedo * _Color * 2.5;
    color.a = saturate(input.color.a * albedo.a * _Color.a * _Intensity);
    color = color * mask1 * mask2;
    color.a = saturate(color.a);
               
    return color;
                
    //half4 color = albedo * mask1 * mask2 * _Intensity;
    //half3 color_rgb = color.r * _Color.rgb * 2.5;
    //half color_a = color.r * _Color.a;
