Shader "URP/UT/Skybox/Procedural"
{
    Properties
    {
        // Fog Mode
    ...
    SubShader
    {
        ...
        v2f vert (appdata_t v)
        {
            ...
            float3 kSkyTintInGammaSpace = FastLinearToSRGB(_SkyTint); // convert tint from Linear back to Gamma
...
//              for(int i=0; i<int(kSamples); i++)
                {
                    ...
                    float lightAngle = dot(lightDirectionWS, samplePoint) / height;
            ...
            else
            {
                ...
                float lightAngle = dot(lightDirectionWS, pos);
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
            ...
            OUT.skyColor    = _Exposure * (cIn * getRayleighPhase(lightDirectionWS, -eyeRay));

        #if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
            // The sun should have a stable intensity in its course in the sky. Moreover it should match the highlight of a purely specular material.
            // This matching was done using the standard shader BRDF1 on the 5/31/2017
            // Finally we want the sun to be always bright even in LDR thus the normalization of the lightColor for low intensity.
            half lightColorIntensity = clamp(length(mainLight.color.rgb), 0.25, 1);
            #if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
                OUT.sunColor    = kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * mainLight.color.rgb / lightColorIntensity;
            #else // SKYBOX_SUNDISK_HQ
                OUT.sunColor    = kHDSundiskIntensityFactor * saturate(cOut) * mainLight.color.rgb / lightColorIntensity;
            #endif
            ...
            #endif
        #endif
            CustomMixFogColor(vertexInput.positionWS, OUT.fogColor.xyz, OUT.fogColor.w);
            return OUT;
        }
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            if(y < 0.0)
            {
                Light mainLight = GetMainLight();
                float3 lightDirectionWS = normalize(mainLight.direction);
                col += IN.sunColor * calcSunAttenuation(lightDirectionWS, -ray);
            }
        #endif
            ...
            //float3 worldPos = mul((float3x3)unity_ObjectToWorld, IN.vertex.xyz);
            col = lerp(IN.fogColor.xyz, col.rgb, IN.fogColor.w);
