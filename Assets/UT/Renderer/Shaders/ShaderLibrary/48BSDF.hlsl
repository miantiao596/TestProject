...
real3 EvalIridescence(real eta_1, real cosTheta1, real iridescenceThickness, real3 baseLayerFresnel0, real iridescenceIntensity = 1, real iorOverBaseLayer = 0.0)
//==================================48Change End==================================//
//=============================================================================//
{
    real3 I;
    ...
    else
    {
        ...
        for (int m = 1; m <= 2; ++m)
        {
            ...
            real3 Sm = 2.0 * EvalSensitivity(m * OPD, m * phi) * iridescenceIntensity;
...
real DV_Anisotropy(real3 normalWS, real3 tangentWS,real3 viewDirectionWS,real perceptualRoughness, real anisotropy, real3 lightDirWS)
{
    float3 H = SafeNormalize(float3(lightDirWS) + float3(viewDirectionWS));
    real3 bitangentWS = cross(normalWS, tangentWS.xyz);

    real NdotL = dot(normalWS, lightDirWS);
    real clampedNdotV = ClampNdotV(dot(normalWS, viewDirectionWS));
    ...
    real TdotV = dot(tangentWS.xyz, viewDirectionWS);
    real BdotH = dot(bitangentWS, H);
    real BdotL = dot(bitangentWS, lightDirWS);
    real BdotV = dot(bitangentWS, viewDirectionWS);

    roughnessT = max(0.01, roughnessT);
    roughnessB = max(0.01, roughnessB);

    real partLambdaV = GetSmithJointGGXAnisoPartLambdaV(TdotV, BdotV, clampedNdotV, roughnessT, roughnessB);
    real DV = DV_SmithJointGGXAniso(TdotH, BdotH, NdotH, clampedNdotV, TdotL, BdotL, abs(NdotL), roughnessT, roughnessB,
                                    partLambdaV);
    return max(0, DV);
}
//==================================48Add End==================================//
