...
void FillMaterialSSS(inout BRDFData bsdfData)
{
    bsdfData.fresnel0 = _TransmissionTintsAndFresnel0.a;
    // bsdfData.materialFeatures |= MATERIALFEATUREFLAGS_SSS_OUTPUT_SPLIT_LIGHTING;
    // bsdfData.materialFeatures |= GetSubsurfaceScatteringTexturingMode(diffusionProfileIndex) << MATERIALFEATUREFLAGS_SSS_TEXTURING_MODE_OFFSET;
}

float4 _DualLobeAndDiffusePower;
bool GetDualLobeParameters(out float multiplierA, out float multiplierB, out float lobeMix)
{
    multiplierA = _DualLobeAndDiffusePower.r;
    multiplierB = _DualLobeAndDiffusePower.g;
    lobeMix     = _DualLobeAndDiffusePower.b;
    return multiplierA != multiplierB; // if both multipliers are equal, there is no dual lobe
}

float GetDiffusePower()
{
    return _DualLobeAndDiffusePower.a;
}
...
// Assume that bsdfData.diffusionProfileIndex is init
void FillMaterialTransmission(float thickness, inout BRDFData bsdfData)
{
    ...
    // Compute transmittance using baked thickness here. It may be overridden for direct lighting
    // in the auto-thickness mode (but is always used for indirect lighting).
    bsdfData.transmittance = ComputeTransmittanceDisney(_ShapeParamsAndMaxScatterDists.rgb,
                                                        _TransmissionTintsAndFresnel0.rgb,
                                                        bsdfData.thickness);
}
