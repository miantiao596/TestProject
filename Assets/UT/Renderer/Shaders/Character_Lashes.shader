...
//        [ShowIf(_ControlMapMode, LGreater, 1)][ShowIf(_OcclusionMapKey, Equal, 1)][Sub(SurfaceInputs)]_OcclusionMap("AO贴图", 2D) = "white" {}
...
//        [ShowIf(_EnableMaterialCapture, Equal, 1)][Sub(SurfaceInputs)][NoScaleOffset]_MatCapMap("映射贴图", 2D) = "black" {}
        ...
        /************Character Base************/
        /*************************************/
        [Main(LashesProperties, _, on, off)] _LashesProperties ("Lashes Properties", float) = 0

        [Sub(LashesProperties)][NoScaleOffset]_LashesTintMap("染色贴图", 2D) = "white" {}
        [Sub(LashesProperties)]_LashesTintColor1("染色色号1", Color) = (1,1,1,0)
        [Sub(LashesProperties)]_LashesTintColor2("染色色号2", Color) = (1,1,1,0)
        [Sub(LashesProperties)]_LashesTintColor3("染色色号3", Color) = (1,1,1,0)
        
        [Space(10)]
        [SubToggle(LashesProperties,_HIGHLIGHT)] _EnableHighlight("区域高亮",Float)=1
        [ShowIf(_EnableHighlight, Equal, 1)][Sub(LashesProperties)][NoScaleOffset]_HighlightMap("高亮贴图", 2D) = "black" {}
        [ShowIf(_EnableHighlight, Equal, 1)][Sub(LashesProperties)]_HighlightMap_ST("UV Tiling and Offset", Vector) = (1, 1, 0, 0)
        [ShowIf(_EnableHighlight, Equal, 1)][SubToggle(LashesProperties)]_HighlightUVMirror("UV镜像", Float) = 0
        [ShowIf(_EnableHighlight, Equal, 1)][Sub(LashesProperties)]_HighlightUVRotate("UV旋转", Range(-1,1)) = 0
        [ShowIf(_EnableHighlight, Equal, 1)][Sub(LashesProperties)]_HighlightIntensity("高亮强度", Range(0,1)) = 1
...
//        [ShowIf(_FGDMapKey, Equal, 1)][Sub(AdvancedOptions)][NoScaleOffset]_FGDMap("FGD贴图", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "ShaderLibrary/Character/Lashes.hlsl"
